...
\subtitle{Soutenance Intermédiaire - Projet Recherche}
...
\begin{frame}{Objectifs du projet}
    Le projet vise à développer un outil de détection temps réel basé sur la \textbf{"Spoofability"} des marchés.
    
    \vspace{0.5cm}
    \textbf{Objectifs principaux :}
    \begin{enumerate}
        \item \textbf{Implémentation d'un modèle probabiliste :} Réseau de neurones estimant la distribution conditionnelle des rendements futurs \cite{fabre_2025}.
        ...
        \end{itemize}
        \item \textbf{Validation de l'hypothèse :} Les marchés actions sont-ils structurellement moins sensibles (spoofables) que les marchés crypto ?
    \end{enumerate}
\end{frame}
...
\begin{frame}{Feature Engineering : Processus de Hawkes}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textit{Temps :} Décroissance exponentielle $e^{-\beta t}$ (mémoire du marché).
            \item \textit{Volume ($v_s$) :} Impact de la taille de l'ordre.
...
\begin{frame}{Détection par la "Spoofability"}
    ...
    \begin{alertblock}{Règle de Détection}
    ...
    \end{alertblock}
    Le modèle PNN sert à calculer ces espérances en prédisant le mouvement de prix $\Delta p$ induit.
\end{frame}

% ==============================================================================
% SECTION 4: AVANCEMENT
% ==============================================================================
\section{Avancement et Plan}
\begin{frame}{Avancement actuel et Plan de travail}
    ...
    \begin{itemize}
        ...
        \item Prise en main des données (Level 3 Crypto fournies par Sun Zu Lab / Encadrant).
        \item Architecture du code Python structurée (pré-traitement, modèle PNN).
    \end{itemize}

    \textbf{Planification (Mois 2-5) :}
    \begin{itemize}
        \item \textbf{Décembre - Janvier :} Implémentation complète et calibration du modèle PNN sur les données Crypto (BTC/ETH).
        \item \textbf{Février :} Récupération et calibration sur données Actions (LOBster ou équivalent) pour comparaison.
        \item \textbf{Mars :} Analyse de sensibilité (Impact de la distance $\delta$ et du volume $Q$ sur la distribution $\Delta p$). Quantification de la spoofability comparée.
...
\section*{Annexe}
\subsection*{Organisation du travail d'équipe}
