...
def parse_feature_attributes(feature_name):
    ...
    if 'bid' in name_lower:
        side = 'bid'
    elif 'ask' in name_lower:
        side = 'ask'
    else:
        side = 'neutral'
    
    # Determine level (for LOB features like bid-price-1, ask-volume-3, ofi_level_1)
    level = None
    
    # Pattern for features like "bid-price-1", "ask-volume-10"
    lob_match = re.search(r'(bid|ask)[-_]?(price|volume)[-_]?(\d+)', name_lower)
    if lob_match:
        level = int(lob_match.group(3))
    
    # Pattern for OFI features like "order_flow_imbalance_level_1"
...
def explain_grouped_occlusion(pipeline, x_seq, feature_names, group_by='side', baseline_mode='mean'):
    """
    ...
    # Build feature list strings for each group
...
def analyze_root_cause_simple(model_pipeline, inputs, model_type='ae', SEQ_LENGTH=25, feature_names=None, device='cuda'):
    """
    Computes feature contribution to the anomaly.
    - AE/PRAE: Squared Reconstruction Error per feature.
    - PNN: Variance (Sigma) per feature.
    - T+OCSVM: N/A (requires occlusion, effectively black box here).
    """
    ...
    if model_type == 'prae':
        with torch.no_grad():
            rec = model_pipeline.model(inputs, training=False)[0]
            # Contribution = MSE per feature
            contrib = torch.mean((inputs - rec) ** 2, dim=1).cpu().numpy()

    elif model_type == 'transformer_ocsvm':
        with torch.no_grad():
            rec = model_pipeline.model(inputs)
            contrib = torch.mean((inputs - rec) ** 2, dim=1).cpu().numpy()

    elif model_type == 'pnn':
        flat = inputs.view(inputs.size(0), -1)
        mu, sigma, alpha = model_pipeline.model(flat)
        
        # We backpropagate the sum of sigmas to get gradients for inputs
        sigma.sum().backward()
        
        # Gradients (Batch, Seq, Feat) -> Mean abs gradient over time
        grads = inputs.grad.abs()
        contrib = torch.mean(grads, dim=1).cpu().numpy()
        
        # Detach to clean up graph
        inputs.requires_grad = False

    # Aggregate top features across the batch of anomalies
    mean_contrib = np.mean(contrib, axis=0)
...
def analyze_subset(pipeline, loader, dataset_name, model_type, feature_names, device='cuda', SEQ_LENGTH=25, save_path=None, use_pot=True, risk_level=1e-3):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if model_type == 'transformer_ocsvm':
                latent = pipeline.model.encoder(batch_x)
                latent_np = latent.cpu().numpy()
                latent_np = pipeline.latent_scaler.transform(latent_np)
                batch_scores = -pipeline.detector.decision_function(latent_np)
                
            elif model_type == 'pnn':
                if batch_x.dim() == 3: batch_flat = batch_x.view(batch_x.size(0), -1)
                else: batch_flat = batch_x
                mu, sigma, alpha = pipeline.model(batch_flat)

                batch_scores = sigma.view(-1).cpu().numpy()
                
            elif model_type == 'prae':
                reconstructed = pipeline.model(batch_x, training=False)
                if isinstance(reconstructed, tuple): reconstructed = reconstructed[0]
                
                batch_scores = torch.mean((batch_x - reconstructed) ** 2, dim=[1, 2]).cpu().numpy()
    ...
    if save_path is not None: plt.savefig(save_path + f"_anomaly_scores.png")
    ...
    if len(anomaly_indices) > 0:
        ...
        sample_size = min(len(anomaly_indices), 50)
        idx_sample = np.random.choice(anomaly_indices, sample_size, replace=False)

        subset_data = loader.dataset.data # Tensor (N, Features)
        
        # reconstruct sequences
        batch_seqs = []
        for i in idx_sample:
            start = loader.dataset.indices[i]
            seq = subset_data[start : start + SEQ_LENGTH]
            batch_seqs.append(seq)
            
        batch_seqs = torch.stack(batch_seqs).to(device)

        try:
            rc_df = analyze_root_cause_simple(pipeline, batch_seqs, model_type=model_type, feature_names=feature_names)
            
            plt.figure(figsize=(8, 4))
            sns.barplot(data=rc_df.head(10), x='Contribution', y='Feature', palette='magma')
            plt.title(f"Root Causes - {dataset_name} ({model_type.upper()})")
            
            if save_path is not None: plt.savefig(save_path + f"_root_causes.png")
...
def fit_pot_threshold(scores, risk_level=1e-3, init_level=0.98):
    """
    Fits a Generalized Pareto Distribution (GPD) to the tail of the scores to determine the threshold.
    
    Args:
        scores: Array of anomaly scores.
        risk_level: Probability of a false positive (e.g., 1e-3 = 0.1%).
        init_level: Percentile to define the initial 'tail' (e.g., Top 2%).
    """
    # Define the initial tail
    threshold_u = np.percentile(scores, init_level * 100)
    peaks = scores[scores > threshold_u] - threshold_u
    
    # Fit GPD to the peaks
    def gpd_log_likelihood(params):
        sigma, gamma = params
        if sigma <= 0 or (gamma != 0 and np.any(1 + gamma * peaks / sigma <= 0)):
            return 1e9
        
        n = len(peaks)
        term1 = n * np.log(sigma)
        term2 = (1 + 1/(gamma + 1e-6)) * np.sum(np.log(1 + gamma * peaks / (sigma + 1e-6)))
        return term1 + term2

    # Find best sigma/gamma
    res = minimize(gpd_log_likelihood, [np.std(peaks), 0.1], method='Nelder-Mead')
    sigma_hat, gamma_hat = res.x
